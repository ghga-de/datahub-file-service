...
Within GHGA Central services, file upload activity is tracked and broadcasted through Kafka events. However, DHFS is not connected to GHGA Central's Kafka network. Instead, a dedicated service at GHGA Central liaises between DHFS instances and the rest of GHGA Central, ensuring that DHFS instances only receive information about files uploaded to their S3 instance. Because this communication occurs through a public-facing HTTP API, DHFS requests are authenticated by DHFS-signed JSON Web Tokens. Each Data Hub must therefore generate an *ES256* JSON Web Key pair and share the public key with GHGA Central. The created private key must be set as the `data_hub_signing_key` configuration value.
...
The second use for Crypt4GH is to securely transfer the *new* file encryption secret to GHGA Central after file re-encryption. This value is set directly as `central_api_crypt4gh_public_key`.
...
```bash
docker pull ghga/datahub-file-service:1.0.3
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/datahub-file-service:1.0.3 .
```
...
docker run -p 8080:8080 ghga/datahub-file-service:1.0.3 --help
```
